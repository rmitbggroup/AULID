...
public:
    ...
    int range_query_len(std::vector<std::pair<T, P>> &results, const T& lower, int len) {
...
private:
    ...
    int range_core_len(std::vector<std::pair<T, P>> &results, int pos, Node* node, const T& lower, int len)
    {
        if constexpr (SATISFY_LOWER) {
            int bit_pos = 0;
            const bitmap_t* none_bitmap = node->none_bitmap;
            while (bit_pos < node->num_items) {
                ...
                while (not_none) {
                    ...
                    int i = bit_pos + latest_pos;
                    if (BITMAP_GET(node->child_bitmap, i) == 0) {
//                        results[pos] = node->items[i].comp.data.value;
                        results.push_back({node->items[i].comp.data.key, node->items[i].comp.data.value});
                        // __builtin_prefetch((void*)&(node->items[i].comp.data.key) + 64);
                        pos ++;
                    } else {
...
//                        results[pos] = node->items[lower_pos].comp.data.value;
                        results.push_back({node->items[lower_pos].comp.data.key, node->items[lower_pos].comp.data.value});
                        pos ++;
                    }
                } else {
